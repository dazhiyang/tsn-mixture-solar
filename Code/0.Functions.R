...
dtsn <- function(x, xi, omega, alpha, a, b, log = FALSE) {
  ...
  sn_dens <- dsn(x, xi = xi, omega = omega, alpha = alpha)
  
  # Truncation constant
  cdf_a <- psn(a, xi = xi, omega = omega, alpha = alpha)
  cdf_b <- psn(b, xi = xi, omega = omega, alpha = alpha)
...
ptsn <- function(q, xi, omega, alpha, a, b, lower.tail = TRUE, log.p = FALSE) {
  ...
  sn_cdf <- psn(q, xi = xi, omega = omega, alpha = alpha)
...
rtsn <- function(n, xi, omega, alpha, a, b) {
  F_a <- psn(a, xi = xi, omega = omega, alpha = alpha)
  F_b <- psn(b, xi = xi, omega = omega, alpha = alpha)
  u <- runif(n, F_a, F_b)
  result <- tryCatch({
    # Code that may produce an error (the "try" block)
    qsn(u, xi = xi, omega = omega, alpha = alpha)
  }, error = function(e) {
    ...
    qsn(u, xi = xi, omega = omega, alpha = alpha, solver = "RFB")
  })
...
#     candidates <- rsn(n * 2, xi = xi, omega = omega, alpha = alpha)
#     valid <- candidates[candidates >= a & candidates <= b]
#     
#     # Add valid samples
#     needed <- n - accepted
#     if (length(valid) > 0) {
#       take <- min(needed, length(valid))
#       samples[(accepted + 1):(accepted + take)] <- valid[1:take]
...
get_tsn3_initial_params <- function(dat) {
  ...
  km <- kmeans(dat, centers = 3, nstart = 25)
  ...
  return(list(
    logit_p1 = qlogis(props[cluster1]),
    logit_p2 = qlogis(props[cluster2]),
    xi1 = tsn1_params$xi,
...
generate_mixture_samples <- function(n, fit) {
  ...
  comps <- sample(1:length(lambda), n, replace = TRUE, prob = lambda)
...
generate_tsn_mixture <- function(n, a, b, fit) {
  ...
  # Sample components for each observation
  comps <- sample(1:length(p), n, replace = TRUE, prob = p)
